...
namespace Bifrost.Client.Specs.Commands.for_CommandForProxies
{
    ...
    public class when_getting_for_command_type : given.all_dependencies
    {
        ...
        Establish context = () =>
        {
            ...
            ).Returns((Type type, Type[] interfaces, ProxyGenerationOptions options) =>
            {
                ...
                return typeof(ProxyType<MyCommand>);
            });
        };

        Because of = () => result = command_for_proxies.GetFor<MyCommand>() as ProxyType<MyCommand>;
